...
class BelumBayarController extends Controller
{
    ...
    {
        $head_transaksi = HeadTransaksi::where('id_agen', Session::get('id'))->where('id_reseller', null)->where('status', 'Menunggu pembayaran')->get();
        $detail_transaksi = DetailTransaksi::with('produk')->get();
        $count = count($head_transaksi);
        //dd($detail_transaksi);

        return view('agen.riwayat pembelian.belum bayar.index', compact('head_transaksi','count','detail_transaksi'));
    }
    ...
    public function detail($no_pesanan)
    {
        ...
        return view('agen.riwayat pembelian.belum bayar.detail', compact('head_transaksi','detail_transaksi','detail_agen'));
    }
}
