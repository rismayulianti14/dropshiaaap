...
class SemuaPesananController extends Controller
{
    ...
    {
        $head_transaksi = HeadTransaksi::where('id_agen', Session::get('id'))->where('id_reseller', null)->get();
        $detail_transaksi = DetailTransaksi::with('produk')->get();
        $count = count($head_transaksi);
        //dd($head_transaksi);

        return view('agen.riwayat pembelian.semua.index', compact('head_transaksi','count','detail_transaksi'));
    }
    ...
    public function detail($no_pesanan)
    {
        ...
        return view('agen.riwayat pembelian.semua.detail', compact('head_transaksi','detail_transaksi','detail_agen','detail_reseller'));
    }
}
