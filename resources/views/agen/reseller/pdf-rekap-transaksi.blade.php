...
<body>
    ...
    <table border="0" width="100%" style="border-collapse: collapse">
        <tr>
            <td width="12%" style="font-size: 15px">ID</td>
            <td style="font-size: 15px" width="2%">:</td>
            <td style="font-size: 15px"><b>{{ $reseller->kode_id }}</b></td>
        </tr>
        <tr>
            <td style="font-size: 15px">Nama</td>
            <td style="font-size: 15px">:</td>
            <td style="font-size: 15px">{{ $reseller->nama_lengkap }} <span style="color: #777; font-size: 13px">( Reseller )</span></td>
        </tr>
        <tr>
            <td style="font-size: 15px">Alamat</td>
            <td style="font-size: 15px">:</td>
            <td style="font-size: 15px">
                {{ $reseller->alamat_detail }}, {{ $reseller->subdistrict_name }}, {{ $reseller->city_name }},<br>
                {{ $reseller->province_name }}, Indonesia, {{ $reseller->kode_pos }}
            </td>
        </tr>
    </table>
    ...
    <table border="1" width="100%" style="border-collapse: collapse; margin-top: 20px">
        ...
        <tbody>
                        ...
                        <tr>
                            <td colspan="4" height="35px">
                                @if($row->id_reseller != null)
                                    <b><span style="color: #000">{{ !empty($row->reseller) ? $row->reseller->nama_lengkap:'' }}</span><span style="font-size: 12px; color: #777"> ( Reseller )</span></b>
                                @else
                        ...
                        <tr>
                            <td style="font-size: 14px; padding: 3px;">
                                @foreach($detail_transaksi as $det)
                                    ...
                                    - {{ $det->produk->nama_produk }} <span style="font-size: 13px; color: #777">( x {{ $det->qty }} )</span><br>
                                    @endif
                                @endforeach
                            </td>
            ...
            @endforeach
            <!--<tr>
                <td colspan="2" align="right; padding: 15px">
                    Total pembelian
                    <b style="margin-left: 50px;">Rp.10.000</b>
                </td>
            </tr>-->
        </tbody>                           
    </table>
