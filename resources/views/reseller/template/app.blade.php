...
<body class="hold-transition layout-top-nav">
	...
	<!-- UPDATE PROFIL RESELLER-->
	<script>
		$('body').on('click', '#editReseller', function (event) {
			...
			$.get('/reseller/profil/' + id + '/get', function (data) {
				console.log(data);
				$.each(data, function(key, val){
					...
					$('#button').html('<button type="submit" id="submitReseller" class="btn bg-orange" style="width: 100%">Simpan</button> ');
					$('#btn_edit').html('<button class="btn btn-md btn-default float-right" id="closeReseller" data-id="{{ Session::get('id') }}"><i class="fa fa-times"></i></button>')
				});
			});
		});

		$('body').on('click', '#closeReseller', function (event) {
			event.preventDefault();
			var id = $(this).data('id');
			console.log(id)

			$('#btn_edit').html('<button class="btn btn-md btn-default float-right" id="edit" data-id="{{ Session::get('id') }}">Edit</button>')
			$("#id").html('<p> {{ Session::get('kode_id') }} </p>');
		...
		$('body').on('click', '#submitReseller', function (event) {
			event.preventDefault();
			...
			$.ajax({
				...
				success : function(data) {
					...
					$('#alert').html('<div id="global-alert" class="alert alert-success alert-dismissible" style="margin: 15px 0 15px 0; opacity: 70%;"> <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> <h4><i class="icon fa fa-check"></i> Berhasil</h4> <i style="margin-left: 42px;">Profil berhasil di ubah</i> </div>');
					$("#id").html('<p> {{ Session::get('kode_id') }} </p>');
					$("#nama_lengkap").html('<p> {{ Session::get('nama_lengkap') }} </p>');
					$("#username").html('<p> {{ Session::get('username') }} </p>');
					$("#email").html('<p> {{ Session::get('email') }} </p>');
